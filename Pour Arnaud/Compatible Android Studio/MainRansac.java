...
public class MainRansac {
    ...
    private static ArrayList<Point> obtentionCoins(Bitmap imageGrise) {
        ...
        ArrayList<Point> data1 = obtenirPoints(imageGrise,0,imageGrise.getWidth(),0,650);
        Ransac ransac1 = new Ransac(fitting,data1,1);
        horizontale1 = appliquerRansac(ransac1,imageGrise);

        ArrayList<Point> data2 = obtenirPoints(imageGrise,0,imageGrise.getWidth(),1850,2500);
        Ransac ransac2 = new Ransac(fitting,data2,1);
        horizontale2 = appliquerRansac(ransac2,imageGrise);


        ArrayList<Point> data3 = obtenirPoints(imageGrise,0,700,0,imageGrise.getHeight());
        Ransac ransac3 = new Ransac(fitting,data3,1);
        verticale1 = appliquerRansac(ransac3,imageGrise);

        ArrayList<Point> data4 = obtenirPoints(imageGrise,1400,1700,0,imageGrise.getHeight());
    ...
    private static ArrayList<Point> obtenirPoints(Bitmap imageGrise, int xmin, int xmax, int ymin, int ymax) {
        ...
        for(int i=xmin; i<xmax-1; i++) {
            for(int j=ymin; j<ymax-1; j++) {
                ...
                int colorOriginal3 = imageGrise.getPixel(i,j) ;
                grisGauche = Color.red(colorOriginal3) ;
                int colorOriginal4 = imageGrise.getPixel(i+1,j) ;
                grisDroite = Color.red(colorOriginal4) ;
