...
public class ApplicationHomographie{	
	...
	public static void main(String[] args) throws IOException{
        ...
        String fichierSortie = "imagebrute_homographie";
	...
	private static BufferedImage partieEntiere(BufferedImage imageBrute){
		...
		EstimationHomographie.H = phi.getHomographyCoefficients(p1,p2,p3,p4,p5,p6,p7,p8);
		
		for(int i=0;i<=7;i++) {
        		System.out.println(EstimationHomographie.H.getValueAt(i,0)) ;
		}
		//Definition de la nouvelle image comme une image vierge
        BufferedImage imageRecalee = new BufferedImage(600,500, imageBrute.getType());
		...
		for(int i=0; i<imageRecalee.getWidth(); i++) {
	            for(int j=0; j<imageRecalee.getHeight(); j++) {  //On parcourt toute l'image
	            	...
	            	Point p = phi.phiInverse(ij, EstimationHomographie.H);
	            	u = (int) p.getX();
	            	v = (int) p.getY();
